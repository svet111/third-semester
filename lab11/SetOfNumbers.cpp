...
SetOfNumbers operator+(const SetOfNumbers &set1, const SetOfNumbers &set2) {
    SetOfNumbers result(set1); // Создаем копию первого множества
    for (int num : set2.numbers) {
        if (!result.containsNumber(num)) {
            result.numbers.push_back(num); // Добавляем элементы из второго множества, если их нет в первом
        }
    }
    return result;
}
...
SetOfNumbers operator*(const SetOfNumbers &set1, const SetOfNumbers &set2) {
    SetOfNumbers result;
    for (int num : set1.numbers) {
        if (set2.containsNumber(num)) {
